class ExercisesController < ApplicationController
  ...
  def exercise3 
    ...
    @address = Address.joins(:orders).group(:id).having("count(*) > ?", 1).order(count: :desc).first
  end
  ...
  def exercise4 
    ...
    @customer = Customer.joins(orders: :foods).group(:id).having("count(*) > ?", 1).order(count: :desc).first
  end
end
